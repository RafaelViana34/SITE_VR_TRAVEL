...
let autoPlayInterval, progressInterval, progressStart, currentSlide = 0,
    isAnimating = !1,
    mouseX = 0,
    mouseY = 0,
    isPausedByUser = !1,
...
function updateParallax() {
    ...
    e && (e.style.transform = `translate(${20*mouseX}px, ${20*mouseY}px)`), t.forEach((e, t) => {
...
function startAutoPlay() { clearInterval(autoPlayInterval), stopProgressBar(), isPausedByUser || (startProgressBar(), autoPlayInterval = setInterval(() => { nextSlide(), stopProgressBar(), startProgressBar() }, slideDuration)) }
...
function startProgressBar() {
    ...
    e && (e.style.width = "0%", progressStart = Date.now(), clearInterval(progressInterval), progressInterval = setInterval(t, 50), t())
}
...
function openOverlay(e) {
    ...
    n && setTimeout(() => { const e = n.querySelector(".scroll-indicator"); if (e) { n.scrollHeight > n.clientHeight && (e.classList.add("show"), n.classList.add("has-scroll")) } }, 100)
}

function closeOverlay(e) { document.getElementById(e + "Overlay").classList.remove("active"), isPausedByUser || startAutoPlay() }

function handleSubmit(e) { e.preventDefault(), alert("Thank you for your message! We will get back to you soon."), e.target.reset(), closeOverlay("contact") }

function moveShapeToRandomPosition(e) {
    const t = 60 * Math.random() + 10,
        n = 80 * Math.random() + 5;
    e.style.top = t + "%", e.style.left = n + "%", e.style.bottom = "auto", e.style.right = "auto";
    const s = 360 * Math.random();
    e.style.transform = `rotate(${s}deg)`, setTimeout(() => { e.style.transform = "" }, 800)
}

function autoMoveShapes() { document.querySelectorAll(".parallax-shape").forEach((e, t) => { setTimeout(() => { moveShapeToRandomPosition(e) }, 2e3 * t) }) }
...
function handleScrollIndicator(e) {
    ...
    function n() {
        ...
        n && !s ? t.classList.add("show") : t.classList.remove("show"), n && !s ? e.classList.add("has-scroll") : e.classList.remove("has-scroll")
    }
    t && (setTimeout(n, 100), e.addEventListener("scroll", n))
}
menuToggle.addEventListener("click", () => { menuToggle.classList.toggle("active"), mainNav.classList.toggle("active") }), document.querySelectorAll("nav a").forEach(e => { e.addEventListener("click", () => { window.innerWidth <= 768 && (menuToggle.classList.remove("active"), mainNav.classList.remove("active")) }) }), slider.addEventListener("mousemove", e => {
    const t = slider.getBoundingClientRect();
    mouseX = (e.clientX - t.left) / t.width - .2, mouseY = (e.clientY - t.top) / t.height - .5, updateParallax()
...
function updateDuration(e) {
    currentDuration = Math.max(1, Math.min(9, e)), slideDuration = 1e3 * currentDuration, durationDisplay.innerHTML = currentDuration + "<span>s</span>";
    const t = getTransitionDuration();
    document.documentElement.style.setProperty("--slide-transition-duration", t + "ms"), isPausedByUser || startAutoPlay()
}
document.documentElement.style.setProperty("--slide-transition-duration", "700ms"), decreaseBtn.addEventListener("click", e => { e.stopPropagation(), updateDuration(currentDuration - 1) }), increaseBtn.addEventListener("click", e => { e.stopPropagation(), updateDuration(currentDuration + 1) });
let uiVisible = !0;
